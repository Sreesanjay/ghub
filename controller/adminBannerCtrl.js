...
const getAllBanners = asyncHandler(async (req, res, next) => {
    ...
    allBanners = allBanners.map(banner => {
        return {
            ...banner.toObject(),
            starting_date: new Date(banner.starting_date).toLocaleDateString(),
            exp_date: new Date(banner.exp_date).toLocaleDateString(),
        };
    });
    res.render('admin/adminBanner', { allBanners,success:req.flash('success')[0]})
})

const newBanner = asyncHandler(async (req, res, next) => {
    let category = await Category.find({}, { cat_name: 1 })
...
const getEditBanner = asyncHandler(async (req, res, next) => {
    ...
    let parsedDate = new Date(banner.starting_date)
    const year = parsedDate.getUTCFullYear();
    const month = String(parsedDate.getUTCMonth() + 1).padStart(2, '0');
    const day = String(parsedDate.getUTCDate()).padStart(2, '0');
    banner.st_date = `${year}-${month}-${day}`

    let parsedExp = new Date(banner.exp_date)
    ...
    let categoryId = banner.category
    banner.category = await Category.findById(banner.category, { cat_name: 1 })
    let category = await Category.find({ _id: { $nin: [categoryId] } }, { cat_name: 1 })
...
const deleteBanner=asyncHandler(async(req, res, next)=>{
   const banner= await Banner.findByIdAndDelete(req.params.id,{new:true})
   if(banner) {
    res.status(200).json({status:'success'})
   }
   else{
