...
const editAddress = asyncHandler(async (req, res, next) => {
  const newAddress = await Address.findByIdAndUpdate(req.params.id, req.body, { new: true })
  if (newAddress) {
    res.status(200).json({ status: 'success' })
  } else {
    throw new Error()
...
const addToWishlist = asyncHandler(async (req, res, next) => {
  ...
  let wishlist;
  const exist = await User.findOne({ _id: user, wishlist: { $elemMatch: { product_id: product_id } } })
  if (exist) {
    wishlist = await User.updateOne({ _id: user }, { $pull: { wishlist: { product_id: product_id } } })
  } else {
    wishlist = await User.updateOne({ _id: user }, { $push: { wishlist: { product_id } } })
  }
  if (wishlist) {
    res.status(200).json({ status: 'success' })
  } else {
...
const deleteWish=asyncHandler(async(req, res, next)=>{
  ...
  let newWishlist= await User.updateOne({ _id: user }, { $pull: { wishlist: { product_id: product_id } } })
  if(newWishlist){
    res.status(200).json({status: 'success'})
  }else{
    throw new Error()
